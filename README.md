...
- **配置持久化**：config.json 存 `%APPDATA%\WinTab`，升级/重装不丢失；旧版 exe 旁配置首次运行自动迁移
...
配置文件 `config.json` 位于 `%APPDATA%\WinTab\`（升级/重装不丢失）。旧版本配置（exe 目录/项目根目录）首次运行自动迁移复制过来，旧文件保留。缺失时自动生成默认配置。
