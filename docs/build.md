...
```bash
# Rust：配置校验/序列化、状态机匹配筛选/分页/程序列表、程序编辑代号冲突、热键 vk 映射、枚举冒烟、版本资源
cd src-tauri && cargo test

# 前端：纯函数（src/util.js：escapeHtml / prettyHotkey / settingsFormEquals /
# rectPhys / clipRectPhys / winHintKind）单测 + 全量 JS 语法检查
node --test            # 或 npm test（package.json）
...
6. 空格快速跳转（最近两个窗口来回切）；程序数超过生效页长时 PageUp/PageDown 翻页；「每页卡片数」步进范围按当前屏幕自动限定（可行区间，重置按钮一键回 scale≈1 的本屏推荐值，默认 20），生效值即所选值——卡片等比缩放（0.75–1.35）铺满屏幕、不过大不过小
7. 设置页（F2）：窗口排序切换、多字母开关、语言/主题切换、版本与双语更新记录显示；未保存返回弹确认（热键录制功能仍暂停，见 TODO 与分支 `hotkey-settings-wip`）
