...
**窗口过滤**：仅计入可见窗口（`IsWindowVisible`），排除自身覆盖层窗口、桌面（`Progman`）、任务栏（`Shell_TrayWnd`）及无标题窗口。

**多窗口编号顺序：Z 序**。`EnumWindows` 的返回顺序即 Z 序（栈顶在前），编号 1 = 最上层窗口，与 Alt+Tab 顺序一致。
